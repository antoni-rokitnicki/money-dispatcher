package pl.org.tosio.slack.money.money

import pl.org.tosio.slack.money.graph.BalanceRelationship
import pl.org.tosio.slack.money.graph.MoneyRelationshipRepository
...
class MoneyServiceTests extends Specification {
    ...
    def "When new users are making payment create one pay and balance relationships"() {
        given:
        def param = new AddDto("1", "2", 10.00, null)
    ...
    def "When user try to make payment to himself throw SelfMoneyException"() {
        given:
        def param = new AddDto("1", "1", 10.00, null)
        when:
        cut.addMoney(param)
        then:
